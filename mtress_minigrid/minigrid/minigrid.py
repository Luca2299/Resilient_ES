...
# enable plots?
enable_mg_plots = False # first false to avoid many plots then later turn true to see plots of last run
...
# days times hours per day,times intervals per hour -> this is an hourly timestep
...
def run_loop(radiation_data):
    ...
    storage_counter = 0
    run_counter = 0
    storage_price = (storage_counter*storage_step+storage_min)*2000 #$/kWh #calculate_storage_price()
    if enable_mg_plots:
    ...
    for i in range(storage_min, storage_max + 1, storage_step):
        ...
        for j in range(pv_min, pv_max + 1, pv_step):
            ...
            print(f'\nRun #{run_counter + 1} '
                  ...
                  f'\nPV size:                  {j} qm\n'
                  f'----------------------------------------\n\n')
            meta_model, capex, co2_emissions = opti_el_supply(j, storage_counter, storage_price)
    ...
    plt.title("CO2 Emissions vs. CAPEX (Colormap by Storage size)")
    plt.xlabel("CAPEX in €")
    ...
    plt.plot(pareto_front['capex'], pareto_front['co2'], marker='+', color='green', alpha = 0.1)
    plt.title("CO2 emissions vs. CAPEX (Colormap by PV panel size in qm)")
    cbar = plt.colorbar()
...
def opti_el_supply(j,storage_counter, storage_price): 
    ...
    meta_model = run_mtress(parameters=yaml_file_name, time_range=(first_time_step, last_time_step))
    capex = np.around((cost_per_qm * j) + (meta_model.operational_costs()) + (storage_price), 2) # previous version (storage_price[storage_counter])
