...
def run_loop(radiation_data):
    ...
    for i in range(storage_min, storage_max + 1, storage_step):
        ...
        for j in range(pv_min, pv_max + 1, pv_step):
            ...
            print(f'\nRun #{run_counter + 1} '
                  ...
                  f'\nPV size:                  {j} qm\n'
                  f'----------------------------------------\n\n')
            meta_model, totalCosts, co2_emissions = opti_el_supply(j, storage_counter, storage_cost)
            ...
            # save Total annual costs in column 1, co2 in column 2 of the dataframe
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(results['totalCosts'], results['co2'], c=results['Storage (kWh)'],
                cmap='jet', marker='+')
    ...
    plt.xlabel("Total Costs in €/year")
...
def opti_el_supply(j,storage_counter, storage_cost): 
    ...
    totalCosts = np.around((cost_per_qm * j)/lifetime_PV + (meta_model.operational_costs()) + (storage_cost)*(lifetime_PV/lifetime_battery)/lifetime_PV, 2) # this are total costs per year 
...
if __name__ == '__main__':
    ...
    # run loop
    run_loop(radiation_data)
>>>>>>> 2dde5c003496adcd34ebc44b2afc5b53f40b56ab
